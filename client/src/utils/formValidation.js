...
const formValidation = (pokemon) => {
  ...
  if (!nameRegex.test(pokemon.name) || pokemon.name === "")
    errors.name = "Nombre del pokemon no puede estar vacío";
  ...
  if (pokemon.image === "")
    errors.image = "El pokemon requiere de una imagen para crearse";

  // Valido que las siguientes características esten completos
  if (pokemon.life <= 0 || isNaN(parseFloat(pokemon.life)))
    errors.life = "Vida del pokemon no puede estar vacío";
  if (pokemon.life > 999)
    errors.life = "La vida del pokemon no puede ser mayor a 999";

  if (pokemon.attack <= 0 || isNaN(parseFloat(pokemon.attack)))
    errors.attack = "Ataque del pokemon no puede estar vacío";
  if (pokemon.attack > 999)
    errors.life = "El ataque del pokemon no puede ser mayor a 999";

  if (pokemon.defense <= 0 || isNaN(parseFloat(pokemon.defense)))
    errors.defense = "Defensa del pokemon no puede estar vacío";
  if (pokemon.defense > 999)
    errors.life = "La defensa del pokemon no puede ser mayor a 999";
  ...
  if (pokemon.speed > 999)
    errors.life = "La velocidad del pokemon no puede ser mayor a 999";
  if (pokemon.weight > 999)
    errors.life = "El peso del pokemon no puede ser mayor a 999";
  if (pokemon.weight > 999)
    errors.life = "La altura del pokemon no puede ser mayor a 999";
