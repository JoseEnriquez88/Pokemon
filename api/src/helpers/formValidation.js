...
const formValidation = (
  ...
  if (!nameRegex.test(name))
    throw new Error(`${name} no es válido como nombre para el pokemon`);
  if (!name.length > 50)
    throw new Error("El nombre no puede contener mas de 50 caracteres");
  if (!name.length === "")
    throw new Error("El nombre es necesario para poder crear el pokemon.");
  ...
  if (!image.length === "")
    throw new Error("La imagen es necesario para poder crear el pokemon.");
  ...
  if (types.length === 0 || types.length > 2)
    throw new Error(
      "Los tipos son necesario para poder crear el pokemon. Digite al menos 1 tipo y máximo 2"
    );
  ...
  if (
    isNaN(parseFloat(life)) ||
    isNaN(parseFloat(attack)) ||
    isNaN(parseFloat(defense))
  )
    throw new Error(
      "Vida, ataque y defensa del pokemon deben ser valores numéricos o decimales"
    );
  if (life === 0 || attack === 0 || defense === 0)
    throw new Error(
      "Vida, ataque y defensa son necesarios para poder crear el pokemon"
    );
  ...
  if (
    ...
    throw new Error(
      "Vida, ataque, defensa, velocidad, peso y altura no puede ser mayor a 999"
    );
};
